<div class="total-line text-grey-300">
    ...
    @if (auth()->user()->is_admin)
        @if (isset($showAverageIncome) && $showAverageIncome)
            {{ __('Average monthly income:') }} <b>{{ round( ($sum - ( ((int)getSettings()['my_status'] || ($data['fix_tax']->year ?? $data['year']) == (int)date('Y')) ? (int)$data['fix_tax'] : 0 ) ) / ($data['year'] == date('Y') ? (int)date('n') : 12) ) }}₽</b><br>
